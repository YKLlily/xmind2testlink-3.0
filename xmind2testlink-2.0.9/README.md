...
```shell
xmind2testlink2 /path/to/testcase.xmind
Generated: testcase.xml

# output json is also supported
xmind2testlink2 /path/to/testcase.xmind -json
...
cd /path/to/xmind2testlink2/web
...
```bash
docker run -d --name xmind2testlink2 --restart always -p 3000:5001 tobyqin/xmind2testlink2
```
...
```bash
cd web
docker build -t xmind2testlink2:latest .
docker run -d --name xmind2testlink2 --restart always -p 3000:5001 tobyqin/xmind2testlink2
...
python -m pip install xmind2testlink2 -U >NUL
